...
async fn main() {
    ...
    // Startup cleanup: restore system to clean state in case of prior crash.
    ...
    let watchdog_state = Arc::clone(&app_state);
    tokio::spawn(async move {
        watchdog::run_watchdog(watchdog_state, Duration::from_secs(10)).await;
    });
    ...
    let app = Router::new()
        ...
        // Watchdog
        .route("/api/watchdog", post(api::set_watchdog))
